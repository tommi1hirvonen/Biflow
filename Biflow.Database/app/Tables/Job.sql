CREATE TABLE [app].[Job] (
    ...
    [JobCategoryId] UNIQUEIDENTIFIER CONSTRAINT [FK_Job_Category] FOREIGN KEY REFERENCES [app].[JobCategory] ([JobCategoryId]) ON DELETE SET NULL,
...
CREATE TRIGGER [app].[Trigger_Job]
    ON [app].[Job]
    INSTEAD OF DELETE
    AS
    BEGIN
        ...
        DELETE FROM app.Step WHERE JobId IN (SELECT JobId FROM deleted) OR JobToExecuteId IN (SELECT JobId FROM deleted)
        DELETE FROM app.JobParameter WHERE JobId IN (SELECT JobId FROM deleted)
        DELETE FROM app.Job WHERE JobId IN (SELECT JobId FROM deleted)

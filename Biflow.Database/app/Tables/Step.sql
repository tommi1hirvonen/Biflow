CREATE TABLE [app].[Step] (
    [JobId]                         UNIQUEIDENTIFIER NOT NULL CONSTRAINT [FK_Step_Job] FOREIGN KEY ([JobId]) REFERENCES [app].[Job] ([JobId]),
    ...
    [ResultCaptureJobParameterId]   UNIQUEIDENTIFIER NULL CONSTRAINT [FK_Step_JobParameter] FOREIGN KEY REFERENCES [app].[JobParameter] ([ParameterId]) ON DELETE SET NULL,
    [ConnectionId]                  UNIQUEIDENTIFIER CONSTRAINT [FK_Step_Connection] FOREIGN KEY REFERENCES app.Connection ([ConnectionId]) NULL,
    ...
    [PipelineClientId]              UNIQUEIDENTIFIER CONSTRAINT [FK_Step_PipelineClient] FOREIGN KEY REFERENCES app.PipelineClient ([PipelineClientId]) NULL,
    [PipelineName]                  NVARCHAR(250)    NULL,
    [JobToExecuteId]                UNIQUEIDENTIFIER NULL CONSTRAINT [FK_Step_JobToExecute] FOREIGN KEY REFERENCES [app].[Job] ([JobId]),
    ...
    [AppRegistrationId]             UNIQUEIDENTIFIER NULL CONSTRAINT [FK_Step_AppRegistration] FOREIGN KEY REFERENCES [app].[AppRegistration] ([AppRegistrationId]),
    [DatasetGroupId]                NVARCHAR(36)     NULL,
    [DatasetId]                     NVARCHAR(36)     NULL,
    [FunctionAppId]                 UNIQUEIDENTIFIER NULL CONSTRAINT [FK_Step_FunctionApp] FOREIGN KEY REFERENCES [app].[FunctionApp] ([FunctionAppId]),
    ...
    [RetryIntervalMinutes]          FLOAT            CONSTRAINT [DF_Step_RetryIntervalMinutes] DEFAULT (0) NOT NULL,
    [TimeoutMinutes]                FLOAT            CONSTRAINT [DF_Step_TimeoutMinutes] DEFAULT (0) NOT NULL,
    ...
    [QlikCloudClientId]             UNIQUEIDENTIFIER CONSTRAINT [FK_Step_QlikCloudClient] FOREIGN KEY REFERENCES [app].[QlikCloudClient] ([QlikCloudClientId]),
...
CREATE TRIGGER [app].[Trigger_Step]
    ON [app].[Step]
    INSTEAD OF DELETE
    AS
    BEGIN
        ...
        DELETE FROM app.Dependency WHERE DependantOnStepId IN (SELECT StepId FROM deleted) OR StepId IN (SELECT StepId FROM deleted)
        DELETE FROM app.Step WHERE StepId IN (SELECT StepId FROM deleted)
    END
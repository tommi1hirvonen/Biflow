...
public class SerializationTests(SerializationTestsFixture fixture) : IClassFixture<SerializationTestsFixture>
{
    ...
    public void Serialize_AzureCredentials()
    {
        ...
        var items = JsonSerializer.Deserialize<AzureCredential[]>(json, EnvironmentSnapshot.JsonSerializerOptionsPreserveReferences);
        Assert.NotNull(items);
        Assert.NotEmpty(items);
        Assert.All(items, x => Assert.NotEqual(x.AzureCredentialId, Guid.Empty));
        // TODO Check sensitive properties for all types inheriting AzureCredential (e.g. client secret, password).

    ...
    : StepExecutor<PackageStepExecution, PackageStepExecutionAttempt>(logger, dbContextFactory)
{
    ...
    protected override PackageStepExecutionAttempt Clone(PackageStepExecutionAttempt other, int retryAttemptIndex) =>
        new(other, retryAttemptIndex);

    protected override async Task<Result> ExecuteAsync(PackageStepExecution step, PackageStepExecutionAttempt attempt, ExtendedCancellationTokenSource cancellationTokenSource)
    {
        ...
        try
        {
            ...
            packageOperationId = await StartExecutionAsync(step, connection.ConnectionString, executeAsLogin);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            while (!completed)
            {
                (completed, success) = await GetStatusWithRetriesAsync(connection.ConnectionString, packageOperationId, linkedCts.Token);
                if (!completed)
        ...
        {
            await CancelAsync(step, attempt, connection.ConnectionString, packageOperationId);
        ...
        if (!success)
        {
            try
            {
                var errors = await GetErrorMessagesAsync(connection.ConnectionString, packageOperationId);
    ...
    private static async Task<long> StartExecutionAsync(PackageStepExecution step, string connectionString, string? executeAsLogin)
    {
    ...
    private async Task<(bool Completed, bool Success)> GetStatusWithRetriesAsync(
